...
<body onload="pageInit();" style='OVERFLOW:AUTO;OVERFLOW-X:HIDDEN'>
 <div id="basicform" class="contentwrapper">
  <form action="${sys_ctx}/meetingRoom/addSave.action" id="modifyForm" name="modifyForm" method="post">
    <input name="" id="meetingRoomID" type="hidden" value="${meetingRoomVO.meetingRoomID}"/>
    
    <div class="contenttitle2">
      <h5 class="fwb fl10">会议室复制</h5>
    </div>
    <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tableadd">
        <tr>
          <td width="15%" class="tableaddtitle"><span>*</span>会议室名称</td>
          <td width="35%" class="tableadd_data" >
            <input class="inputtran" style="width:80%" type="text" name="meetingRoomVO.meetingRoomName" id="meetingRoomName" value="" onblur="javascript:check('meetingRoomName');" />
          </td>
          <td width="15%" class="tableaddtitle"><span>*</span>会议室类型</td>
          <td width="35%" class="tableadd_data">
            <select class="inputtran" name="meetingRoomVO.meetingRoomType" id="meetingRoomType" >
				<zzst:option type="meetingRoomType" value="${meetingRoomVO.meetingRoomType}" required="true"/>
			</select>
          </td>
        </tr>
        <tr>
          ...
          <input class="inputtran" onclick="showMenu();" id="name"  name="meetingRoomVO.addressVO.name" type="text" readonly value='<c:out value="${meetingRoomVO.addressVO.name }"></c:out>' />
            <input class="inputtran" id="addressID"  name="meetingRoomVO.addressVO.addressID" type="hidden" value="${meetingRoomVO.addressVO.addressID }" />
          ...
          <td class="tableadd_data">
            <input type="text" class="inputtran" name="meetingRoomVO.departmentVO.title" id="title" readonly="readonly"  value="${meetingRoomVO.departmentVO.title}" onclick="selectDepartments(this);"/>
            <input type="hidden" class="inputtran" name="meetingRoomVO.departmentVO.id" id="departmentId"  value="${meetingRoomVO.departmentVO.id}" />
        ...
        <tr>
          ...
          <td class="tableadd_data" >
                 <select class="inputtran" name="meetingRoomVO.status" id="status" >
					<zzst:option type="meetingRoomStatus" value="${meetingRoomVO.status }" required="true"/>
				</select></td>
          <td class="tableaddtitle"><span>*</span>管理员</td>
          <td class="tableadd_data">
            
            <input type="text" class="inputtran" name="meetingRoomVO.userVO.name" id="names" readonly="readonly" value="${meetingRoomVO.userVO.name}" onclick="selectUsers(this);"/>
            <input type="hidden" name="meetingRoomVO.userVO.userID" id="userID" class="" value="${meetingRoomVO.userVO.userID}"/></td>
        </tr>
        <tr>
          <td class="tableaddtitle"><span>*</span>容纳人数</td>
          <td class="tableadd_data" ><input class="inputtran" type="text" name="meetingRoomVO.capacity" id="capacity"  value="${meetingRoomVO.capacity}"></td>
          <td class="tableaddtitle"><span>*</span>会议室编号</td>
          <td class="tableadd_data"><input class="inputtran" type="text" name="meetingRoomVO.roomNO" id="roomNO"  value="" onblur="checkNO(this)" /></td>
        </tr>
        <tr>
          <td class="tableaddtitle">会议室描述</td>
          <td colspan="3" class="tableadd_data" ><textarea name="meetingRoomVO.description" rows="5" class="areatran" id="description">${meetingRoomVO.description}</textarea></td>
        </tr>
      </table>
      <table cellpadding="0" cellspacing="0" border="0" class="buttoncontainer" id="table">
        ...
        <tbody>
          <tr>
            <td>
              <input type="button" name="button" id="button" class="submit1 radius2 submitBtn_Disa" value="确 定" onclick="return MeetingRoomModify()"/>
