...
<script type="text/javascript">
     ...
     if(selId=="picShow"){
    	 window.location.href="${sys_ctx}/meeting/meetingManage/viewCompare/detailCompareView.jsp?meetingRoomNo="+meetingRoomID+"&meetingDetailId="+meetingDetailId+"&compareDetailId="+compDetailID+"&equipIp="+masterEpIp;
         }
     else{
	...
	<body class="withvernav">
		...
		<select class="radius3" id="selId"  onchange="toview('<%=equipIp %>')" >
			<option value="tableShow"  >列表模式</option>
			<option value="picShow" selected="selected">图表模式</option>
		</select> 
		...
		<input type="hidden" id="meetingRoomID" value="<%=meetingRoomNo %>"/>
