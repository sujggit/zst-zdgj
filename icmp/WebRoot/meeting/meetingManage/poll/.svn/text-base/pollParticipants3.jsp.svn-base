...
<html>
  ...
  <body onload="venueSortable()">
    ...
    <div id="screenContainer" class="participantsContainerDiv">
      <div id="participantsContainer1" style="width:99%;height:50%;" class="participants">
	    ...
	    <ul id="terList1">
		   	<c:forEach items="${tList1}" var="ter1" varStatus="k">
			   	<li id="ter1${k.count }">
				   	...
				   	<input type="button" value="删除" class="participantsDelBtn" class="participantsDelBtn" onclick="delTerminal('ter1${k.count}')"/>
			   		<span class="participantsSpan">${ter1.mcu_participant_name }</span>
			   	</li>
		   	</c:forEach>
	   	</ul>
     </div>
     <div id="participantsContainer2" style="width:50%;height:50%;" class="participants">
	    ...
	    <ul id="terList2">
		   	<c:forEach items="${tList2}" var="ter2" varStatus="s">
		   		<li id="ter2${s.count }">
			   		...
			   		<input type="button" value="删除" class="participantsDelBtn" class="participantsDelBtn" onclick="delTerminal('ter2${s.count}')"/>
		   			<span class="participantsSpan">${ter2.mcu_participant_name }</span>
		   		</li>
	   		</c:forEach>
	   	</ul>
     </div>
     <div id="participantsContainer3" style="width:49%;height:50%;" class="participants">
	    ...
	    <ul id="terList3">
		   	<c:forEach items="${tList3}" var="ter3" varStatus="s">
		   		<li id="ter3${s.count }">
			   		...
			   		<input type="button" value="删除" class="participantsDelBtn" class="participantsDelBtn" onclick="delTerminal('ter3${s.count}')"/>
