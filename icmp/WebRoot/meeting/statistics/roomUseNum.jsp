...
<script type="text/javascript" src="${sys_ctx }/js/alertJS/zDialog.js"></script>
		...
		function exportRoomNum(){
        //获得模式 add by tanzanlong
        	var modeChange = $("#modeChange").val();
			document.getElementById('pageform').action="${sys_ctx}/statistics/exportNumberQuery.action?modeChange="+modeChange;
			document.getElementById('pageform').submit();
			document.getElementById('pageform').action="${sys_ctx}/statistics/queryNum.action";
		}
 ...
 <div id="basicform" class="contentwrapper">
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tableadd">
        <tr>
          ...
          <td width="35%" class="tableadd_data">
         <input type="text" id="startTime" name="meetingRoomVO.startTime" 	readonly onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'});"   value='<fmt:formatDate value="${meetingRoomVO.startTime }"  pattern="yyyy-MM-dd HH:mm"/>'/>
          --        
           <input type="text" id="endTime" name="meetingRoomVO.endTime" 	readonly onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm'});"   value='<fmt:formatDate value="${meetingRoomVO.endTime }"  pattern="yyyy-MM-dd HH:mm"/>'/>
           </td>
    ...
    <div id="contentwrapper" class="contentwrapper">
      <div class="contenttitle2">
       ...
       <select id="modeChange"  onchange="modeSelect();">
        ...
        <h5 class="fwb fr10"><a style="cursor:pointer" onclick="exportRoomNum();">导出</a></h5>
      </div>
      ...
      <table cellpadding="0" cellspacing="0" border="0" class="stdtable" id="query_table">
        <thead>
          <tr>
            ...
            <th width="24%" class="head1">使用次数</th>
          </tr>
        </thead>
        <tfoot>
        </tfoot>
        <tbody>
           <c:forEach items="${listMeetingRoom}" var="meetingRoomVO"
									varStatus="state">
									<tr>
										<td class="alc">
										...
										<td>
											<c:out value="${meetingRoomVO.useCount}" />
										</td>
									</tr>
								</c:forEach>
