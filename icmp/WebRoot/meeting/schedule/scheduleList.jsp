...
<html>
  <head>
    ...
    <%-- <%@include file="/common/funcOperPower.jsp"%> --%><!-- 权限管理用？？？ -->
    <title>领导日程查询</title>
	...
	<script type="text/javascript">
		...
		function modify(scheduleTime,createTime){
			$("#scheduleTime").val(scheduleTime);
			$("#createTime").val(createTime);
			$("#subForm").attr("action","/icmp/schedule/scheduleModifyBefore.action");
			//alert(createTime);
			$("#subForm").submit();
		}
  ...
  <body>
    ...
    <br>
    <hr size="2px;">
    <form action="${sys_ctx }/schedule/meetingScheduleQuery.action" id="pageform" name="pageform" method="post">
		  ...
		  <c:forEach items="${scheduleVOList }" var="schedule" varStatus="state">
		  	<tr>
	  			...
	  			<td>
	  				...
	  				<c:if test="${sys_userSession.userRoleVOList[0].roleID=='a' }">|</c:if>
	  				<c:if test="${sys_userSession.userRoleVOList[0].roleID=='a' }">
						<a class="funcOper" onclick="javascript:modify('${schedule.scheduleTime }','${schedule.createTime }');">修改</a>
					</c:if>
	  			</td>
	  		</tr>
		  	</c:forEach>
