...
<body style='OVERFLOW:AUTO;OVERFLOW-X:HIDDEN'>
  <form action="${sys_ctx }/file/addMeetingFile.action" enctype="multipart/form-data" id="form" name="form" method="post" target="iframeTemp" >
     <input type="hidden" value="${param.id}" name="uploadFileVO.uploadKey"/>
  ...
  <script type="text/javascript">
	<%
	  ...
	  out.println("var name = '" + name + "'");  
	 %>
	 document.getElementById("titleDiv").innerHTML = "文件上传（所属会议室：" + name + "）";
	...
	function addMore(){
		...
		$(td2).append("<img src='${sys_ctx }/images/dtree_images/empty.gif' title='' style='float: right' id='fileIco"+index+"'/>");		
	...
	function upload(index){
		...
		*/
		...
		//if(reg.test(document.getElementById("fileUrlTemp"+index).value)){
			$("#form").attr("action","${sys_ctx }/fileupload?savePath=/file/meetingRoom/&maxSize=20&index="+index);
	...
	function uploadDataBase(){
		...
		if(undefined != fileUrls[0]){
			...
			if(fileUrls[0].value){
				...
				if(flag){
					$(":file").attr("disabled","true");
					$("#form").attr("action","${sys_ctx }/file/addMeetingRoomFile.action?fileUrlArr="+fileUrlArr);
