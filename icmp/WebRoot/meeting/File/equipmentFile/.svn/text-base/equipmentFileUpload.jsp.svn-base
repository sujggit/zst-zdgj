...
<body style='OVERFLOW:AUTO;OVERFLOW-X:HIDDEN'>
  ...
  <script type="text/javascript">
	<%
	 ...
	 %>
	 document.getElementById("titleDiv").innerHTML = "文件上传（所属设备：" + name + "）";
	...
	function upload(index){
		...
		*/
		...
		//if(reg.test(document.getElementById("fileUrlTemp"+index).value)){
			$("#form").attr("action","${sys_ctx }/fileupload?savePath=/file/equipment/&maxSize=20&index="+index);
	...
	function uploadDataBase(){
		...
		if(undefined != fileUrls[0]){
			...
			if(fileUrls[0].value){
				...
				if(flag){
					$(":file").attr("disabled","true");
					$("#form").attr("action","${sys_ctx }/file/addEquipmentFile.action?fileUrlArr="+fileUrlArr);
