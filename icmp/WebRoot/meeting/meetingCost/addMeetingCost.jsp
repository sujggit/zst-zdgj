...
<head>
	...
	<div id="basicform" class="contentwrapper">
		<form action="${sys_ctx }/meetingCost/meetingCostList.action" method="post" name="form" id="form">
          ...
          <div class="contenttitle2">
            <h5 class="fwb fl10">会议费用增加（只能输入整数；单位：人*天）</h5>
          </div>
          <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tableadd">
	        ...
	        <tr>
	          ...
	          <td class="tableadd_data">
	            <input type="hidden" id="costItem${state.index+1}" name="meetingDetailCostVO.costItem" value="${dictionaryVO.dicViewName }" />
	            <input type="hidden" id="costValue${state.index+1}" name="meetingDetailCostVO.costValue" value="${dictionaryVO.dicValue }"/>
	            <input type="text" class="inputtran" id="cout${state.index+1}" name="meetingDetailCostVO.cout" />
	          ...
	          <td class="tableadd_data" >
	            <input class="inputtran" disabled="disabled" value="${dictionaryVO.dicValue }"/>
	          </td>
	        </tr>
          </c:forEach>
          </table>
          <table cellpadding="0" cellspacing="0" border="0" class="buttoncontainer" id="table">
            ...
            <tbody>
              <tr>
                <td>
                  <input type="button" class="submit1 radius2 submitBtn_Disa" value="确 定" onclick="addMeetingCost();"/>
  ...
  <script>
    function addMeetingCost(){
		...
		var flag = 0;//20131212 bug840查看和修改的时候会议费用项会重复xiongshun
		for(var i=1;i<=size;i++){
			...
			//20131212 bug840查看和修改的时候会议费用项会重复xiongshun
			flag += cout;
		...
		if(flag==0){//20131212 bug840查看和修改的时候会议费用项会重复xiongshun
			location.href = "${sys_ctx }/meetingCost/meetingCostList.action";
			return;
		}
    	DwrMethod.addMeetingCost(meetingDetailId,createUserID,costArr,callback);
    }
