...
<html>
	  		...
	  		function checkMeetingRoomCallBack(lst){
	  			if(lst != null && lst.length > 0){
  					...
  					//var actionOld = $('#modifyForm').attr("action");
  	 				...
  	 				$('#modifyForm').attr("action","${sys_ctx }/detail/modifyConference.action?depIds="+depIds);
  ...
  <form action="${sys_ctx}/detail/modifyConference.action" id="modifyForm" name="modifyForm" method="post">
   ...
   <input name="meetingDetailVO.status" value="${meetingDetailVO.status}"  type="hidden"/>
    <div>
                  ...
                  DictionaryVO dvs=(DictionaryVO)dvList.get(i);
                   ...
                   <!-- 修改成预约人  info1字段原来存贮会议标签，先存储申请人-->
                   <tr>
                   	<td class="tableaddtitle" >申请人</td>
                   	<td class="tableadd_data" colspan="<%=nums %>">
                   		<input  name="meetingDetailVO.info1" value="${meetingDetailVO.info1 }" id="info1" type="text" class="areatran" readonly="readonly"/>
                   	</td>
                   </tr>
                                            <tr>
				...
				<script type="text/javascript">
					function selectDepartments(thisDom){
						var selectedInfo = document.getElementById("corpID").value;
			               	...
			               	$("#confDocAdminName").attr("value",userName);
                                          ...
                                          <table cellpadding="0" cellspacing="0" border="0" class="buttoncontainer" id="table">
                                            ...
                                            <tbody>
                                              <tr>
                                                <td>
                                               		 <input type="button" name="button" id="button" value="修改会议" onclick="addMeeting()"  class="submit1 radius2" />
