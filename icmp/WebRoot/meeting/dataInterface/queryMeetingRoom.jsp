...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		function closeBg(){ 
			...
			*/
			function meetingRoomDetail(id){
				window.showModalDialog("${sys_ctx }/meetingroomInterface/detail.action?mrVO.id="+id,window,'dialogWidth=600px;dialogHeight=470px;');
			...
			*/
			function meetingRoomModify(id){
				window.location.href="${sys_ctx }/meetingroomInterface/beforeModify.action?mrVO.id="+id;
			...
			*/
			function meetingRoomDele(id){
				...
				window.location.href="${sys_ctx }/meetingroomInterface/delete.action?mrVO.id="+id;
			}
			/**
			*查询会议室 详细 信息 
			*@param		${String}	id
			...
			*/
			function meetingRoomqeury(){
	          ...
	          function pageInit(){
					...
					obj.title("meetingRoomName","请输入长度不超过25个字符");	
				}
			
			function exportRoom(){
				document.getElementById('pageform').action="${sys_ctx}/meetingRoom/exportQuery.action";
				document.getElementById('pageform').submit();
				document.getElementById('pageform').action="${sys_ctx}/meetingroomInterface/queryMeetingRoom.action";
			}

			var type1 = "meetingRoom";
	...
	<body style=OVERFLOW:AUTO;OVERFLOW-X:HIDDEN onload="pageInit()">
		<form action="${sys_ctx}/meetingroomInterface/queryMeetingRoom.action" id="pageform" name="pageform" method="post">
			  ...
			  <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tableadd" id="">
                 <tr>
                   ...
                   <td width="35%" class="tableadd_data" >
			         <input class="inputtran" title="请输入会议室关键字" type="text" name="mrVO.meetingroomName" id="meetingRoomName"  value="${mrVO.meetingroomName}" />
				   </td>
                   <td width="15%" class="tableaddtitle">操作状态</td>
                   <td width="35%" class="tableadd_data">
                   <select  class="" name="mrVO.status" id="status">
					   <zzst:option type="importStatus" value="${mrVO.status}" required="false" />
				   </select></td>
                   <td class="tableaddtitle"><input type="button" class="stdbtn mlr10" value="查 询" onclick="meetingRoomqeury();" /></td>
                </tr>
             </table>
      ...
      <div class="contenttitle2">
        ...
        <h5 class="fwb fr10"><a class="funcOper <%= FuncEnum.FUNC_NO_IMPORTEXCEL%>" style="cursor: pointer;" onclick="importExcel();"> 导入</a> <a class="funcOper <%= FuncEnum.FUNC_NO_DATAIMPORT%>" style="cursor: pointer;" onclick="dataImport();"> 数据同步</a> <a class="funcOper <%= FuncEnum.FUNC_NO_EXPORT%>" style="cursor: pointer;" onclick="exportRoom();"> 导出</a></h5>
      </div>
      ...
      <table cellpadding="0" cellspacing="0" border="0" class="stdtable" id="query_table">
        <thead>
          <tr>
            ...
            <th width="12%" class="head1">会议室名称</th>
            <th width="12%" class="head1">会议室类型</th>
            <th width="12%" class="head1">会议室管理员</th>
            <th width="12%" class="head1">会议室位置</th>
            <th width="12%" class="head1">所属单位</th>
            <th width="9%" class="head1">操作状态</th>
            <th width="12%" class="head1">描述</th>
            <th width="18%" class="head1">操作</th>
          </tr>
        </thead>
		  <tbody>
			<c:forEach items="${mrifList}" var="meetingRoomInterface" varStatus="state">
				<tr>
					<td class="alc">
						<c:out value="${state.index+1}"></c:out>
					</td>
					<td>
						<c:out value="${meetingRoomInterface.meetingroomName }" />
					</td>
					<td>
						<zzst:lable type="meetingRoomType" value="${meetingRoomInterface.meetingRoomType}"></zzst:lable>
					</td>
					<td>
						<c:out value="${meetingRoomInterface.adminName}" />
					</td>
					<td>
						<c:out value="${meetingRoomInterface.addressName}" />
					</td>
					<td>
						<c:out value="${meetingRoomInterface.departmentName}" />
					</td>
					<td
						<c:if test="${meetingRoomInterface.status == 1}">style="color: green;"</c:if>	
						<c:if test="${meetingRoomInterface.status == 2}">style="color: red;"</c:if>
					>
						<zzst:lable type="importStatus" value="${meetingRoomInterface.status}"></zzst:lable>
					</td>
					<td>
						<c:out value="${meetingRoomInterface.description}" />
					</td>
					<td class="alc">
						<a class="funcOper <%= FuncEnum.FUNC_NO_DETAIL%>" onclick="javascript:meetingRoomDetail('${meetingRoomInterface.id}');"/>查看   
						<a class="funcOper <%= FuncEnum.FUNC_NO_MODIFY%>" onclick="javascript:meetingRoomModify('${meetingRoomInterface.id}');" />修改   
						<a class="funcOper <%= FuncEnum.FUNC_NO_DELETE%>" onclick="javascript:meetingRoomDele('${meetingRoomInterface.id}');"/> 删除
					</td>
				</tr>
			</c:forEach>
		  </tbody>
