...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		function closeBg(){ 
			...
			*/
			function departmentDetail(id){
				window.showModalDialog("${sys_ctx }/departmentInterface/detail.action?diVO.departmentid="+id,window,'dialogWidth=600px;dialogHeight=470px;');
			...
			*/
			function departmentModify(id){
				window.location.href="${sys_ctx }/departmentInterface/beforeModify.action?diVO.departmentid="+id;
			...
			*@return	null	
			*/
			function departmentDele(id){
				if(!window.confirm("是否确认删除？")) return;
				window.location.href="${sys_ctx }/departmentInterface/delete.action?diVO.departmentid="+id;
			}
			/**
			*查询会议室 详细 信息 
			*@param		${String}	id
			...
			*/
			function departmentqeury(){
	          ...
	          function pageInit(){
					...
					obj.title("departmentname","请输入长度不超过25个字符");	
				}
			
			function exportDepartment(){
				document.getElementById('pageform').action="${sys_ctx}/departmentInterface/exportQuery.action";
				document.getElementById('pageform').submit();
				document.getElementById('pageform').action="${sys_ctx}/departmentInterface/queryDepartment.action";
			}

			var type1 = "department";
			...
			function dataImport(){
				...
				DataInterfaceDwr.departmentImport(
					function backa(){
						closeBg();
						window.location.reload();
					}
	...
	<body style=OVERFLOW:AUTO;OVERFLOW-X:HIDDEN onload="pageInit()">
		<form action="${sys_ctx}/departmentInterface/queryDepartment.action" id="pageform" name="pageform" method="post">
			<div id="basicform" class="contentwrapper">
			  <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tableadd" id="">
                 <tr>
                   <td width="15%" class="tableaddtitle">组织结构名称</td>
                   <td width="35%" class="tableadd_data" >
			         <input class="inputtran" title="请输入会议室关键字" type="text" name="diVO.departmentname" id="departmentname"  value="${diVO.departmentname}" />
				   </td>
                   <td width="15%" class="tableaddtitle">操作状态</td>
                   <td width="35%" class="tableadd_data">
                   <select  class="" name="diVO.importstatus" id="status">
					   <zzst:option type="importStatus" value="${diVO.importstatus}" required="false" />
				   </select></td>
                   <td class="tableaddtitle"><input type="button" class="stdbtn mlr10" value="查 询" onclick="departmentqeury();" /></td>
                </tr>
             </table>
      ...
      <div class="contenttitle2">
        ...
        <h5 class="fwb fr10"><a class="funcOper <%= FuncEnum.FUNC_NO_IMPORTEXCEL%>" style="cursor: pointer;" onclick="importExcel();"> 导入</a> <a class="funcOper <%= FuncEnum.FUNC_NO_DATAIMPORT%>" style="cursor: pointer;" onclick="dataImport();"> 数据同步</a> <a class="funcOper <%= FuncEnum.FUNC_NO_EXPORT%>" style="cursor: pointer;" onclick="exportDepartment();"> 导出</a></h5>
      </div>
      ...
      <table cellpadding="0" cellspacing="0" border="0" class="stdtable" id="query_table">
        <thead>
          <tr>
            ...
            <th width="19%" class="head1">组织结构名字</th>
            <th width="19%" class="head1">父组织结构名字</th>
		  ...
		  <tbody>
			<c:forEach items="${diList}" var="diVO" varStatus="state">
				<tr>
					<td class="alc">
						<c:out value="${state.index+1}"></c:out>
					</td>
					<td>
						<c:out value="${diVO.departmentname}" />
					</td>
					<td>
						<c:out value="${diVO.parentname}" />
					</td>
					<td
						<c:if test="${diVO.importstatus == 1}">style="color: green;"</c:if>	
						<c:if test="${diVO.importstatus == 2}">style="color: red;"</c:if>
					>
						<zzst:lable type="importStatus" value="${diVO.importstatus}"></zzst:lable>
					</td>
					<td>
						<c:out value="${diVO.importdesc}" />
					</td>
					<td class="alc">
						<a class="funcOper <%= FuncEnum.FUNC_NO_DETAIL%>" onclick="javascript:departmentDetail('${diVO.departmentid}');"/>查看<!--    
						<a class="funcOper <%= FuncEnum.FUNC_NO_MODIFY%>" onclick="javascript:departmentModify('${diVO.departmentid}');" />修改   
						<a class="funcOper <%= FuncEnum.FUNC_NO_DELETE%>" onclick="javascript:departmentDele('${diVO.departmentid}');"/> 删除 -->
					</td>
				</tr>
			</c:forEach>
		  </tbody>
