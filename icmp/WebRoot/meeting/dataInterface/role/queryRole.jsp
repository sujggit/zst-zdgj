...
<html xmlns="http://www.w3.org/1999/xhtml">
		...
		function closeBg(){ 
			...
			*/
			function roleDetail(id){
				window.showModalDialog("${sys_ctx }/roleInterface/detail.action?riVO.roleid="+id,window,'dialogWidth=600px;dialogHeight=470px;');
			...
			*/
			function roleModify(id){
				window.location.href="${sys_ctx }/roleInterface/beforeModify.action?riVO.roleid="+id;
			...
			*@return	null	
			*/
			function roleDele(id){
				if(!window.confirm("是否确认删除？")) return;
				window.location.href="${sys_ctx }/roleInterface/delete.action?riVO.roleid="+id;
			}
			/**
			*查询会议室 详细 信息 
			*@param		${String}	id
			...
			*/
			function roleqeury(){
	          ...
	          function pageInit(){
					...
					obj.title("rolename","请输入长度不超过25个字符");	
				}
			
			function exportRoom(){
				document.getElementById('pageform').action="${sys_ctx}/roleInterface/exportQuery.action";
				document.getElementById('pageform').submit();
				document.getElementById('pageform').action="${sys_ctx}/roleInterface/queryRole.action";
			}
	...
	<body style=OVERFLOW:AUTO;OVERFLOW-X:HIDDEN onload="pageInit()">
		<form action="${sys_ctx}/roleInterface/queryRole.action" id="pageform" name="pageform" method="post">
			<div id="basicform" class="contentwrapper">
			  <table width="100%" border="0" cellspacing="1" cellpadding="0" class="tableadd" id="">
                 <tr>
                   <td width="15%" class="tableaddtitle">角色</td>
                   <td width="35%" class="tableadd_data" >
			         <input class="inputtran" title="请输入会议室关键字" type="text" name="riVO.rolename" id="rolename"  value="${riVO.rolename}" />
				   </td>
                   <td width="15%" class="tableaddtitle">操作状态</td>
                   <td width="35%" class="tableadd_data">
                   <select  class="" name="riVO.importstatus" id="status">
					   <zzst:option type="importStatus" value="${riVO.importstatus}" required="false" />
				   </select></td>
                   <td class="tableaddtitle"><input type="button" class="stdbtn mlr10" value="查 询" onclick="roleqeury();" /></td>
                </tr>
             </table>
      ...
      <table cellpadding="0" cellspacing="0" border="0" class="stdtable" id="query_table">
        <thead>
          <tr>
            ...
            <th width="25%" class="head1">角色</th>
			<th width="25%" class="head1">操作状态</th>
            <th width="26%" class="head1">描述</th>
            <th width="18%" class="head1">操作</th>
          </tr>
        </thead>
		  <tbody>
			<c:forEach items="${risList}" var="rivo" varStatus="state">
				<tr>
					<td class="alc">
						<c:out value="${state.index+1}"></c:out>
					</td>
					<td>
						<c:out value="${rivo.rolename}" />
					</td>
					<td 
						<c:if test="${rivo.importstatus == 1}">style="color: green;"</c:if>	
						<c:if test="${rivo.importstatus == 2}">style="color: red;"</c:if>
					>
						<zzst:lable type="importStatus" value="${rivo.importstatus}"></zzst:lable>
					</td>
					<td>
						<c:out value="${rivo.importdesc}" />
					</td>
					<td class="alc">
						<a class="funcOper <%= FuncEnum.FUNC_NO_DETAIL%>" onclick="javascript:roleDetail('${rivo.roleid}');"/>查看   
						<a class="funcOper <%= FuncEnum.FUNC_NO_MODIFY%>" onclick="javascript:roleModify('${rivo.roleid}');" />修改   
						<a class="funcOper <%= FuncEnum.FUNC_NO_DELETE%>" onclick="javascript:roleDele('${rivo.roleid}');"/> 删除
					</td>
				</tr>
			</c:forEach>
		  </tbody>
