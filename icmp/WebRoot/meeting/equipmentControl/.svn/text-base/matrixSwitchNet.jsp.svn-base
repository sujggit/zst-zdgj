...
function changeBg(id,op){
	if(op=="in"){
		...
		TerminalDwrMethod.matriSwitchOutNet(ccip,matriID,id,matriSwitchOutBack);
	}else{
...
function matrixSwitch(){
	if(input == ""){
    	...
    	*/
       TerminalDwrMethod.matriSwitchesByNet(ids,id2s,input,outputValues,matriSwitchssback);
	}
}
...
<div id="k2" class="k" style="display:block">
	...
	<form action="${sys_ctx}/equipmentControl/matrixNetControlBefore.action" id="pageform" name="pageform" method="post">
	...
	<select name="equipmentVO.equipmentID" id="matrixNames" style="margin-top:5px" onchange="matrixName();">
		<c:forEach items="${equipmentList}" var="matrixSwitchVOTemp" varStatus="state">
   		<option value="${matrixSwitchVOTemp.equipmentID}" ${matrixSwitchVOTemp.equipmentID==matrixSwitchVO.id  ? "selected" : "" } ><c:out value="${matrixSwitchVOTemp.equipmentNO}"/></option>
        </c:forEach>
	</select>
