	...
	function sysPwback(lst)
	{
		for(var i=0;i<lst.length;i++)
		{
			...
			if(status=="<%=SysPowerVO.status_on%>")
			{
				...
				$("#openpw").attr("style","background:url(${sys_style1 }/images/ans.png) no-repeat center;color:#fff");
			...
			else if(status=="<%=SysPowerVO.status_off%>")
			{
			  	...
			  	$("#closepw").attr("style","background:url(${sys_style1 }/images/ans.png) no-repeat center;color:#fff");
...
<body onload="go()">
	...
	<div class="k4" id="sysPowerDiv" style="width:80%;display:none">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td><a id="openpw" onclick="pwopen('${ccVO.ip}')">打开</a>&nbsp;&nbsp;<a id="closepw" onclick="pwclose('${ccVO.ip}')">关闭</a></td>
  ...
  <script>
	runit();//加载即执行此方法
	function pwopen(id){//系统电源开
	     DwrMethod.openpowerbyIP(id,function(para){
	    	 if(para !=null){
	        	 var aa = "pw"+para;	 
	...
	function pwclose(id){//系统电源关
	   DwrMethod.closepowerbyIP(id,function(para){
		   if(para !=null){
		    	 var aa = "pw"+para;	 
