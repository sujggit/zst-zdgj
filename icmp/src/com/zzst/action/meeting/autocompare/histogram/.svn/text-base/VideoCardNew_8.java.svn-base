...
public class VideoCardNew_8 {
    ...
    public VideoCardNew_8(String ip){
    	this.ip = ip;
    }
    
    public VideoCardNew_8(String ip, int port){
    	this.ip = ip;
    	this.port = port;
    }
    ...
    * @throws
    ...
    private int[][] sendCommandTCP(byte[] sendByte,String ip,int port,int timeout, int sleepTime)throws Exception {
		...
		if(rcvlen > 512){
			...
			int ii = 0;
			for(int i=2; i<2+512; ){
				rgbArray[0][ii] = ((revByte2[i] & 0xff) << 8) + revByte2[i+1];
		...
		if(rcvlen > 512){
			...
			int ii = 0;
			for(int i=2; i<2+512; ){
				rgbArray[1][ii] = ((revByte3[i] & 0xff) << 8) + revByte3[i+1];
		...
		if(rcvlen > 512){
			...
			int ii = 0;
			for(int i=2; i<2+512; ){
				rgbArray[2][ii] = ((revByte4[i] & 0xff) << 8) + revByte4[i+1];
    ...
    public static void main(String[] args){
		...
		VideoCardNew_8 vcn = new VideoCardNew_8( "10.1.6.49");
