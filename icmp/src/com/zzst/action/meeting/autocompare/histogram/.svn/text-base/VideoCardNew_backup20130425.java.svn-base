...
public class VideoCardNew_backup20130425 {
    ...
    public VideoCardNew_backup20130425(String ip){
    	this.ip = ip;
    }
    
    public VideoCardNew_backup20130425(String ip, int port){
    	this.ip = ip;
    	this.port = port;
    }
	...
	* @throws
    ...
    public void setCommand(byte[] setCommand) throws Exception{
			...
			byte[] revByte = this.sendCommandTCP(setsendbytes,ip,port,timeout, 0);
    ...
    * @throws
    ...
    public byte[] getCommand(byte getCommand) throws Exception{
		...
		byte[] revByte = this.sendCommandTCP(getsendbytes,ip,port,timeout, GET_HISTOGRAM_SLEEP_TIME);
		System.out.println("接收到的返回值："+revByte[2]+"_"+revByte[3]+"_"+revByte[4]+"_"+revByte[7]);
		return revByte;
    }
    /**
    ...
    * @throws
     */
    private byte[] sendCommandTCP(byte[] sendByte,String ip,int port,int timeout, int sleepTime)throws Exception {
		...
		/*
		System.out.println("R len:"+rcvlen);
		System.out.println("G len:"+rcvlen3);
		System.out.println("B len:"+rcvlen4);
		
		if(rcvlen > 512){
			int[] iArray = new int[256];
			int ii = 0;
			for(int i=2; i<2+512; ){
				iArray[ii] = ((revByte2[i] & 0x0ff) * 256) + (revByte2[i+1] & 0x0ff);
				buffer.append(iArray[ii]);
				ii++;
				i = i+2;
				if(i < 2+512){
					buffer.append(",");
		...
		*/
		...
		if(rcvlen > 512){
			...
			int[] iArray = new int[256];
			for(int i=0; i<256; i++ ){
				iArray[i] = ((revByte2[2*(i+1)] & 0x0ff) * 256) + (revByte2[2*(i+1)+1] & 0x0ff);
		...
		if(rcvlen3 > 512){
			...
			int[] iArray = new int[256];
			for(int i=0; i<256; i++ ){
				iArray[i] = ((revByte3[2*(i+1)] & 0x0ff) * 256) + (revByte3[2*(i+1)+1] & 0x0ff);
				buffer.append(iArray[i]);
				buffer.append(",");
			}
		...
		if(rcvlen4 > 512){
			...
			int[] iArray = new int[256];
			for(int i=0; i<256; i++ ){
				iArray[i] = ((revByte4[2*(i+1)] & 0x0ff) * 256) + (revByte4[2*(i+1)+1] & 0x0ff);
				buffer.append(iArray[i]);
				buffer.append(",");
			}
		...
		/*
		System.out.println("len2:"+rcvlen3);
		if(rcvlen3 > 512){
			buffer = new StringBuffer();
			int[] iArray = new int[256];
			int ii = 0;
			for(int i=2; i<2+512; ){
				iArray[ii] = ((revByte3[i] & 0x0ff) << 8) + revByte3[i+1];
				buffer.append(iArray[ii]);
				ii++;
				i = i+2;
				if(i < 2+512){
		...
		System.out.println("len3:"+rcvlen4);
		if(rcvlen4 > 512){
			buffer = new StringBuffer();
			int[] iArray = new int[256];
			int ii = 0;
			for(int i=2; i<2+512; ){
				iArray[ii] = ((revByte4[i] & 0x0ff) << 8) + revByte4[i+1];
				buffer.append(iArray[ii]);
				ii++;
				i = i+2;
				if(i < 2+512){
    ...
    public static void main(String[] args){
		...
		VideoCardNew_backup20130425 vcn = new VideoCardNew_backup20130425("10.1.6.49");
