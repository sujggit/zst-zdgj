...
public class VideoCardNew_DJZ {
	
    public String ip="10.1.6.49";
    public int port=7777;
    public int timeout=3000;
    private VideoCardNew_DJZ(){
    	
    }
    public VideoCardNew_DJZ(String ip){
    	this.ip = ip;
    }
    
    public VideoCardNew_DJZ(String ip, int port){
    	this.ip = ip;
    	this.port = port;
    }
	/** 
    ...
    * @throws
    ...
    private byte[] sendCommandTCP(byte[] sendByte,String ip,int port,int timeout)throws Exception {
		...
		byte[] revByte=new byte[1655];//接收到得数组
	    //发送命令
    ...
    private static void parseData(byte[] revByteArray){
    	if(revByteArray == null || revByteArray.length <= 0){
    		return;
    	}
    	for(byte revByte : revByteArray){
    		System.out.println(revByte + ",");
    	}
    	StringBuffer buffer = new StringBuffer();
    	if(revByteArray[1] == 0 && revByteArray[39] == 1 && revByteArray[578] == 2 && revByteArray[1107] == 3){
    		for(int i=40; i<40+512; i++){
    ...
    public static void main(String[] args){
		...
		VideoCardNew_DJZ vcn = new VideoCardNew_DJZ();
		try {
			//发送视频参数命令设置命令
			...
			//视频评价结果请求
			byte getByte = 0x00;
			byte[] array = vcn.getCommand(getByte);
			parseData(array);
		} catch (Exception e) {
