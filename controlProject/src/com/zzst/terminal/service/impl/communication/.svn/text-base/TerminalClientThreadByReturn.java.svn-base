 ...
 *@author ryan
 */
public class TerminalClientThreadByReturn extends Thread{
	 ...
	 */
	public TerminalClientThreadByReturn(String ip, int port,String command,String commandReturn,boolean waitMark) {
		this.ip = ip;
		this.port = port;
...
//		try {
//			String enueKey = "return";
//			Socket socket = ServerSocketEnues.getTerminalSoket(ip + enueKey);
//			
//			if (socket == null) {
//				socket = new Socket();
...
//							m = false;
//							//System.out.println("==false==return==="+n+"=="+s);
//							break;						
//						}
//						//System.out.println("==true==return==="+n+"=="+s);	
//					}else{
//							m = false;
//							//System.out.println("==false==return==="+n+"=="+s);
//							break;						
...
//		logger.info(str);
//		logger.info("======="+str.indexOf(commandReturn));
//		if(str!=null&&str.length()>0){
//			String[] s = str.split("\r\n");
//			for(String ss : s){
//				System.out.println("返回值：：：："+ss);
//			}
//		}
//		if(str.indexOf(commandReturn)>=0){
			if(str!=null&&str.length()>0){
//				logger.info("有信息返回");
				//提取音量值
				// modify by ryan on 20140328 独立的线程级socket链接处理
//				setAudiometer(str,commandReturn);
				//提取通话信息
				getCallParticularInfo(str);	
				//终端基本信息
				setWhoami(str,commandReturn);
				//提取声音状态
//				getMuteStatus(str);
				//提取摄像头的坐标
				getCameraPosition(str,commandReturn);
			}
		return false;
	}
